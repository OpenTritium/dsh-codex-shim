window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function dispose$1() {
			...
			document.querySelector(`style[data-plugin-css='${tag$1}']`)?.remove();
		}
		...
		*/
		...
		function iconFor(toolName) {
			switch (toolName) {
				...
				case "web_run": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconGlobeOutline14, { size: 14 });
		...
		function webSource(value) {
			...
			const source = value;
			if (typeof source.url !== "string") return void 0;
		...
		function webCardFromBlock(block) {
			if (!("kind" in block) || block.resultView?.card !== "web") return void 0;
			const view = block.resultView;
			if (view.kind === "search") {
				if (!Array.isArray(view.sources) || typeof view.truncated !== "boolean") return void 0;
				const sources = view.sources.map(webSource);
				if (sources.some((source) => source === void 0)) return void 0;
				if (view.answer !== void 0 && typeof view.answer !== "string") return void 0;
				return {
					kind: "search",
					sources,
					truncated: view.truncated,
					...view.answer === void 0 ? {} : { answer: view.answer }
				};
			}
			if (view.kind === "searches" && Array.isArray(view.results)) {
				const results = view.results.map((group) => {
					if (typeof group !== "object" || group === null || Array.isArray(group)) return void 0;
					const item = group;
					if (typeof item.query !== "string" || item.query.trim() === "" || typeof item.truncated !== "boolean" || !Array.isArray(item.sources)) return void 0;
					const sources = item.sources.map(webSource);
					if (sources.some((source) => source === void 0)) return void 0;
					if (item.answer !== void 0 && typeof item.answer !== "string") return void 0;
					return {
						query: item.query,
						sources,
						truncated: item.truncated,
						...item.answer === void 0 ? {} : { answer: item.answer }
					};
				});
		...
		*/
		function CodexToolRow({ toolName, block, inspect, t, imageLoader, useSession }) {
			...
			const web = toolName === "web_run" ? webCardFromBlock(block) : void 0;
			const planItems = useSession((snapshot) => plan === void 0 ? [] : relevantPlanItems(snapshot, block.callId, plan));
			const showRawPanels = toolName !== "view_image" && plan === void 0 && web === void 0;
			const expandable = diff !== null || image !== void 0 || plan !== void 0 || web !== void 0 || showRawPanels && (argsRaw !== "" || output !== null);
				...
				children: [status !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							diff === null && web !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.WebBlock, {
								...web,
								className: _opentritium_css_module_L2hvbWUvdHJpdGl1bS9zcmMvb3BlbnRyaXRpdW0tZHNoLWNvZGV4LXNoaW0vc3JjL2NsaWVudC9Db2RleFRvb2xSb3cubW9kdWxlLmNzcw_default.webBody
							}) : null,
		...
		*/
								...
								addingModel && state.modelsStatus === "ready" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
											...
											disabled,
											onChange: (event) => {
												const selected = state.addableModels.find((row) => modelRouteKey(row.provider, row.model) === event.target.value);
											...
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
												...
												value: modelRouteKey(row.provider, row.model),
												children: [
													row.providerName,
													" / ",
													row.modelName
												]
											}, modelRouteKey(row.provider, row.model)))]
										}),
		...
		function modelRows(groups, overrides) {
			const known = new Map(groups.flatMap((group) => group.models.map((model) => [modelRouteKey(group.id, model.id), {
		...
		function addableModels(groups, overrides) {
			...
			return groups.flatMap((group) => group.models.filter((model) => !overridden.has(modelRouteKey(group.id, model.id))).map((model) => ({
				provider: group.id,
				providerName: group.name,
				model: model.id,
				modelName: model.name
		...
		var CodexSettingsCardController = class {
			...
			field(field) {
				...
				return {
					text: draft ?? (field === "enabled" ? current.enabled === void 0 ? "" : String(current.enabled) : formatPatterns(current.modelPatterns)),
					overridden: this.cleared.has(field) ? false : draft !== void 0 || snapshot.user !== void 0 && field in snapshot.user,
