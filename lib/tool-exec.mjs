...
async function readImage(ctx, exec, path, plugin) {
...
const MAX_OUTPUT_TOKENS = 1048576 / APPROX_BYTES_PER_TOKEN;
/**
...
var ExecSessionRegistry = class {
	...
	*/
	...
	evictionCandidate(owned) {
		...
		return sessions.find((session) => session.proc.status !== "running") ?? sessions[0];
	}
	/**
...
function apply(ctx) {
	...
	ctx.inject(["attachments", "fs"], (imageCtx) => {
		imageCtx.tools.register(defineTool({
			...
			execute: (args, exec) => readImage(imageCtx, exec, args.path, "tool-codex-exec"),
