...
export type ModelOverride = CodexModelOverride
...
export class CodexSettingsCardController {
  ...
  private field(field: 'enabled' | 'modelPatterns'): FieldState {
    ...
    return {
      ...
      overridden: this.cleared.has(field)
        ? false
        : draft !== undefined || (snapshot.user !== undefined && field in (snapshot.user as object)),
  ...
  private snapshot(): CodexSettingsState {
    ...
    return {
      ...
      modelOverridesOverridden:
        !this.cleared.has('modelOverrides') &&
        current.user !== undefined &&
        'modelOverrides' in (current.user as object),
  ...
  getStore(): SnapshotStore<CodexSettingsState> {
    return this.store
  }
